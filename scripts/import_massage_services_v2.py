...
massage_services = [
    ...
    {
        "name": "하와이라이아 핑크 스톤",
        "description": "천연 하와이라이아 소금을 스톤 모양으로 제작해 프리미엄 오일을 발라 피부에 미네랄이 홡수되어 두통과 피부 미용에 효과적한 핫 스톤 마사지",
...
def import_massage_services():
    ...
    try:
        ...
        for service in massage_services:
            ...
            duration_options = ", ".join([f"{d['duration']}분: ₱{d['price']}" for d in service['duration_options']])
