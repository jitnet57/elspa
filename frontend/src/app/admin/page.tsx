  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (sb) {
        ...
        try { const {data:pu} = await sb.from('bookings').select('id').eq('booking_date',today).eq('status','pending'); if(pu)s.pickupRequests=pu.length; } catch{} // bookings 엔 created_at 없음 → booking_date 사용(400 방지)
