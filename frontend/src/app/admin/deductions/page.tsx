...
const TABS: { key: TabKey; label: string }[] = [
  { key: 'ca', label: '선지급(CA)' },
  { key: 'health', label: '보건검진(실비)' },
  { key: 'thirteenth', label: '13개월 선지급' },
  { key: 'sss', label: 'SSS 기여표' },
];

// 에러 메시지 추출 헬퍼
const errMsg = (e: unknown) =>
  e instanceof Error ? e.message : typeof e === 'string' ? e : '알 수 없는 오류가 발생했습니다.';
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        {/* 헤더 */}
        <div className="mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-1">💸 공제/선지급 관리</h1>
          <p className="text-sm text-slate-400">선지급(CA), 보건검진 실비, 13개월 선지급, SSS 기여표를 관리합니다.</p>
        </div>
...
function Status({ loading, error }: { loading: boolean; error: string | null }) {
  if (loading) return <div className="py-8 text-center text-slate-400 text-sm">불러오는 중...</div>;
  if (error)
    return (
...
function CashAdvanceTab() {
  ...
  return (
    <div className="space-y-4">
      {/* 등록 폼 */}
      <div className={card}>
        <h2 className="text-sm font-semibold text-slate-300 mb-3">선지급 등록</h2>
        <div className="flex flex-wrap gap-2">
          <input className={inputCls} type="number" placeholder="직원ID" value={form.employee_id}
            onChange={(e) => setForm({ ...form, employee_id: e.target.value })} />
          <input className={inputCls} type="number" placeholder="금액" value={form.amount}
            onChange={(e) => setForm({ ...form, amount: e.target.value })} />
          <input className={inputCls} type="text" placeholder="사유" value={form.reason}
        ...
        <table className="min-w-full divide-y divide-slate-700">
          ...
          <tbody className="divide-y divide-slate-700">
            {rows.length === 0 && !loading ? (
              <tr><td className="px-3 py-6 text-center text-slate-500 text-sm" colSpan={4}>등록된 선지급이 없습니다.</td></tr>
            ) : (
              rows.map((r) => (
...
function HealthCheckTab() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('이 보건검진 기록을 삭제할까요?')) return;
    setError(null);
    try {
  ...
  return (
    <div className="space-y-4">
      <div className={card}>
        <h2 className="text-sm font-semibold text-slate-300 mb-3">보건검진 실비 등록</h2>
        <div className="flex flex-wrap gap-2">
          <input className={inputCls} type="number" placeholder="직원ID" value={form.employee_id}
            onChange={(e) => setForm({ ...form, employee_id: e.target.value })} />
          ...
          <input className={inputCls} type="number" placeholder="금액" value={form.amount}
            onChange={(e) => setForm({ ...form, amount: e.target.value })} />
          <input className={inputCls} type="text" placeholder="비고" value={form.note}
      ...
      <div className={tableWrap}>
        <table className="min-w-full divide-y divide-slate-700">
          ...
          <tbody className="divide-y divide-slate-700">
            {rows.length === 0 && !loading ? (
              <tr><td className="px-3 py-6 text-center text-slate-500 text-sm" colSpan={5}>등록된 기록이 없습니다.</td></tr>
            ) : (
              rows.map((r) => (
                <tr key={r.id} className="hover:bg-slate-800/40">
                  ...
                  <td className={tdCls}><button className={btnDelete} onClick={() => handleDelete(r.id)}>삭제</button></td>
                </tr>
              ))
            )}
          </tbody>
...
function ThirteenthTab() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('이 13개월 선지급 기록을 삭제할까요?')) return;
    setError(null);
    try {
  ...
  return (
    <div className="space-y-4">
      <div className={card}>
        <h2 className="text-sm font-semibold text-slate-300 mb-3">13개월 선지급 등록</h2>
        <div className="flex flex-wrap gap-2">
          <input className={inputCls} type="number" placeholder="직원ID" value={form.employee_id}
            onChange={(e) => setForm({ ...form, employee_id: e.target.value })} />
...
function SssTab() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('이 SSS 구간을 삭제할까요?')) return;
    setError(null);
    try {
  ...
  return (
    <div className="space-y-4">
      <div className={card}>
        ...
        <div className="flex flex-wrap gap-2">
          <input className={inputCls} type="number" placeholder="구간 하한" value={form.salary_from}
            onChange={(e) => setForm({ ...form, salary_from: e.target.value })} />
          <input className={inputCls} type="number" placeholder="구간 상한" value={form.salary_to}
            onChange={(e) => setForm({ ...form, salary_to: e.target.value })} />
          <input className={inputCls} type="number" placeholder="직원 부담" value={form.employee_share}
            onChange={(e) => setForm({ ...form, employee_share: e.target.value })} />
          <input className={inputCls} type="number" placeholder="사업주 부담" value={form.employer_share}
      ...
      <div className={tableWrap}>
        <table className="min-w-full divide-y divide-slate-700">
          <thead className="bg-slate-800">
            <tr>
              <th className={thCls}>급여 구간</th>
              <th className={thCls}>직원 부담</th>
              <th className={thCls}>사업주 부담</th>
              <th className={thCls}></th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-700">
            {rows.length === 0 && !loading ? (
              <tr><td className="px-3 py-6 text-center text-slate-500 text-sm" colSpan={4}>등록된 구간이 없습니다.</td></tr>
            ) : (
              rows.map((r) => (
                <tr key={r.id} className="hover:bg-slate-800/40">
