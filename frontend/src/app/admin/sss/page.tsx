      ...
      <div className="bg-gradient-to-r from-indigo-500 to-blue-600 text-white px-6 py-6 shadow-lg">
        <div className="max-w-6xl mx-auto flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold">📋 SSS Contribution Management</h1>
            <p className="text-indigo-100 text-sm mt-0.5">
              View · Edit · Scan receipts → Excel
            </p>
          </div>
          {/* 탭 전환 */}
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-indigo-100 p-5">
              <div className="flex flex-wrap items-center gap-4">
                {/* 월 드롭다운 */}
                <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Month</p>
                  {loadingMonths ? (
                    <div className="h-10 w-48 bg-gray-100 rounded-lg animate-pulse" />
                  ) : months.length === 0 ? (
                    <p className="text-sm text-gray-400 italic">No data — scan receipts first</p>
                  ) : (
                    <select
                      value={selectedMonth}
                ...
                {currentMeta && (
                  <>
                    ...
                    <div>
                      <p className="text-xs text-gray-400">Employer SS No.</p>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-indigo-100 overflow-hidden">
              {loadingRecords ? (
                ...
                <div className="p-12 text-center text-gray-400">
                  ...
                  <p className="text-sm">No records for {selectedMonth}</p>
                  <button
                    onClick={() => setTab('scan')}
                ...
                <div className="p-12 text-center text-gray-400">
                  <div className="text-4xl mb-3">📋</div>
                  <p className="text-sm">No SSS data yet. Scan receipts to get started.</p>
                </div>
              ) : (
                <>
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="bg-indigo-50 border-b border-indigo-100">
                          ...
                          <th className="px-3 py-3 text-left font-bold text-gray-600 min-w-[220px]">Name of Employee</th>
                          <th className="px-3 py-3 text-center font-bold text-gray-600 min-w-[140px]">SS Number</th>
                          <th className="px-3 py-3 text-right font-bold text-gray-600 min-w-[110px]">SS (PHP)</th>
                          <th className="px-3 py-3 text-right font-bold text-gray-600 min-w-[90px]">EC (PHP)</th>
                          <th className="px-3 py-3 text-right font-bold text-gray-600 min-w-[110px]">Total (PHP)</th>
                          <th className="px-3 py-3 text-center font-bold text-gray-600 w-20">Save</th>
                          <th className="px-3 py-3 text-center font-bold text-gray-600 w-10">Del</th>
                        </tr>
                      </thead>
                      <tbody>
                        {records.map((row, idx) => {
                        ...
                        <tr className="bg-indigo-600 text-white">
                          <td colSpan={3} className="px-3 py-3 font-bold text-sm">
                            ...
                            <span className="text-indigo-200 font-normal text-xs">
                              ({records.filter(r => r.total_amount > 0).length} with contribution)
                            </span>
                          </td>
                  ...
                  <div className="border-t border-indigo-100 px-5 py-4 bg-indigo-50/50">
                    <div className="flex flex-wrap gap-6 text-sm">
                      <div>
                        <span className="text-gray-500 text-xs">SS Contribution</span>
                        <p className="font-bold text-indigo-800">₱ {peso(totalSS)}</p>
                      </div>
                      <div>
                        <span className="text-gray-500 text-xs">EC Contribution</span>
                      ...
                      <div>
                        <span className="text-gray-500 text-xs">Zero-contribution</span>
                        <p className="font-bold text-red-500">
                          {records.filter(r => r.total_amount === 0).length} employees
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-6 max-w-3xl mx-auto">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-indigo-100 overflow-hidden">
              <div className="px-6 py-4 bg-indigo-50 border-b border-indigo-100 flex items-center gap-3">
                ...
                <h2 className="font-bold text-gray-800">Select SSS Receipt Images</h2>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-indigo-100 overflow-hidden">
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                >
                  {scanStatus === 'scanning' ? (
                    <><span className="animate-spin text-xl">⟳</span> AI Scanning… (20–40 sec)</>
                  ) : (
                ...
                {scanStatus === 'scanning' && (
                  <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <p className="text-sm font-semibold text-indigo-700">Claude AI is reading the receipts…</p>
                    </div>
                    <div className="w-full bg-indigo-100 rounded-full h-2 overflow-hidden">
                      <div className="h-2 bg-indigo-500 rounded-full animate-pulse" style={{ width: '70%' }} />
                    </div>
                    <p className="text-xs text-indigo-400 mt-2">
                      Extracting employee names, SS numbers, and amounts → saving to DB
                    </p>
                  </div>
                )}
                ...
                {scanStatus === 'done' && (
                  <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-green-700">Scan complete!</p>
                      <p className="text-xs text-green-600 mt-0.5">
                        Excel downloaded · Data saved to database · Switched to Records tab
                      </p>
                    </div>
                  </div>
                )}
                ...
                <p className="text-xs text-gray-400 text-center">
                  ⚠ Scanning will replace existing data for the same month
                </p>
              </div>
            </div>
          </div>
