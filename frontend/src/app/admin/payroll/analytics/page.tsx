  ...
  const loadAnalyticsData = async () => {
    ...
    try {
      ...
      const [trendRes, distRes, dedRes, summaryRes, detailRes] = await Promise.all([
        fetch(`/api/payroll/analytics/monthly-trend?year=${selectedYear}`, { headers }),
        fetch(`/api/payroll/analytics/employee-distribution?period_id=${selectedPeriod}`, { headers }),
        fetch(`/api/payroll/analytics/deduction-breakdown?period_id=${selectedPeriod}`, { headers }),
        fetch(`/api/payroll/analytics/summary?period_id=${selectedPeriod}`, { headers }),
        fetch(`/api/payroll/analytics/employee-details?period_id=${selectedPeriod}&skip=0&limit=50`, { headers })
      ]);
