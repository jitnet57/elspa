...
export default function RequirementsPage() {
  ...
  const sections = [
    ...
    },
    {
      ...
      subsections: [
        ...
        },
        {
          title: '(2) 인력 낭비',
          items: [
            '매니저 2명이 상시 기록 업무에 매달려 있음',
            '그 중 1명은 거의 기록 업무 전담',
            '리셉션 3명이 출근 체크와 예약 응대 병행',
            '대표가 정산·인원 예측·직원 관리에 한 달의 대부분 소진'
          ]
        },
        {
          title: '(3) 데이터 분산 & 통합 부재',
          items: [
        ...
        },
        {
          title: '(4) 재정 위험',
          items: [
            '정산 누락 및 기억 의존으로 인한 매출 누수',
            '가이드 커미션: 한 달 뒤에 정산하러 옴 → 분실·누락 위험',
            '수기 바우처 발행 → 기록 누락 가능성 높음',
            '정확한 실시간 재무 현황 파악 불가능'
          ]
        }
      ]
    },
    {
      ...
      subsections: [
        {
          title: '1차: 작업량 1/10 축소',
          items: [
            '매니저의 수기 입력·예측·정산 노동을 1/10 수준으로',
            '매니저 2명 중 최소 1명을 다른 업무로 전환 가능',
            '예약 응대 자동화로 리셉션 운영 효율화'
          ]
        },
        {
          title: '2차: 재정 투명성 확보',
    ...
    },
    {
      ...
      subsections: [
        ...
        },
        {
          ...
          items: [
            'QR 1개 → 구글 로그인 연동',
            '국적 자동 인식 (로컬/한국/가이드 가격 자동 분기)',
            '다국어 자동 응대 (한·영·일·중·필리피노)',
            '바우처 디지털화 (QR 코드 기반 영수증)'
          ]
        },
        {
        ...
        {
          title: '7️⃣ 드라이버 GPS & 배차',
          items: [
            'GPS 기반 드라이버 실시간 위치 추적',
            '손님 요청 → 가장 가까운 드라이버 자동 배차',
            'Pick & Drop 기록 자동 입력',
            '드라이버별 하루 수익·운행 거리 자동 계산'
          ]
        },
        {
          title: '8️⃣ 오프라인 우선 구조',
          items: [
            '필리핀의 느린 인터넷 환경 고려',
            '앱이 오프라인에서도 데이터 입력 가능',
            '와이파이/3G 연결 시 자동 동기화',
            '데이터 충돌 시 자동 병합 로직'
          ]
        },
        {
          title: '9️⃣ 향후 확장',
          items: [
            '동일 구조를 카페 사업부에 복제 (테이블 예약, 메뉴 주문)',
            '네일 사업부에 확대 (스타일리스트 예약, 네일 디자인 갤러리)',
            '투어 사업부와 연동 (패키지 예약 + 교통 편성)',
            '멀티 로케이션 지원 (다른 지역 지점 추가)'
          ]
        }
      ]
    },
    {
      ...
      subsections: [
        ...
        },
        {
          title: '인프라 & 기술스택',
          items: [
            '백엔드: Node.js / Python (AI 에이전트)',
            '데이터베이스: PostgreSQL + Redis (캐싱)',
            '프론트엔드: React Native (iOS/Android)',
            '클라우드: AWS / Google Cloud (필리핀 로컬 서버 옵션)',
            'AI/ML: OpenAI API, TensorFlow (수요 예측, 매칭 알고리즘)'
          ]
        },
        {
          title: '보안 & 컴플라이언스',
          items: [
            '암호화: 모든 데이터 HTTPS 전송 및 DB 암호화',
            '접근제어: 역할별 권한 관리 (RBAC)',
            '감사 로그: 모든 거래 기록 및 인증 추적',
            '필리핀 노동법: 급여, 세금, 보험 자동 계산'
          ]
        }
      ]
    },
    ...
    },
    {
      ...
      subsections: [
        ...
        },
        {
          title: '전략적 성장',
          items: [
            '대표 시간 확보 → 신규 시장(중국·일본) 마케팅 집중',
            '부대사업(카페·네일) 확장 용이',
            '데이터 기반 가격 책정 및 프로모션',
            '장기 목표: 필리핀 내 체인 확대 가능'
          ]
        }
      ]
    },
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 mb-8">
          {sections.map((section) => (
            ...
            >
              {section.title.split(' ')[0]} {section.title.split(' ').slice(1).join(' ').substring(0, 8)}
            </button>
          ))}
        </div>
          ...
          {sections.map((section) => (
            <div
              ...
              className={`bg-slate-800/50 border border-slate-700 rounded-xl overflow-hidden transition-all ${
                expandedSection === section.id ? 'ring-2 ring-cyan-500/50' : ''
              }`}
            >
              {/* Section Header */}
              <button
              ...
              {expandedSection === section.id && (
                <div className="px-6 pb-6 border-t border-slate-700/50">
                  {'content' in section ? (
                    <div className="whitespace-pre-line text-slate-300 leading-relaxed font-mono text-sm">
                      {section.content}
                    </div>
                  ) : (
                          ...
                          <div key={idx}>
                            ...
                            <ul className="space-y-2">
                              {subsection.items.map((item, itemIdx) => (
                                ...
                                >
                                  <span className="text-cyan-400 font-bold min-w-fit mt-1">•</span>
                                  <span>{item}</span>
                                </li>
                              ))}
                            </ul>
        ...
        {/* Footer */}
        <div className="mt-12 p-8 bg-gradient-to-r from-cyan-500/10 to-blue-500/10 border border-cyan-500/30 rounded-xl">
          <h3 className="text-xl font-bold text-cyan-400 mb-3">🎯 프로젝트 목표 달성 체크리스트</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center gap-3">
              <span className="text-2xl">✅</span>
              <span className="text-slate-300">매니저 작업 1/10로 축소</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-2xl">✅</span>
              <span className="text-slate-300">정산 누락 0%</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-2xl">✅</span>
              <span className="text-slate-300">실시간 데이터 통합</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-2xl">✅</span>
              <span className="text-slate-300">대표 경영 집중 가능</span>
            </div>
          </div>
        </div>
      </div>
