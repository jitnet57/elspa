...
// .wrangler/tmp/bundle-ArHKcU/strip-cf-connecting-ip-header.js
...
app.get("/api/bookings", async (c) => {
  try {
    ...
    const { data, error } = await supabase.from("massage_bookings").select("*").order("booking_date", { ascending: false }).limit(100);
    if (error)
      throw error;
...
app.post("/api/bookings", async (c) => {
  try {
    const body = await c.req.json();
    const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_KEY);
    const { data, error } = await supabase.from("massage_bookings").insert([body]).select();
    if (error)
      throw error;
...
app.get("/api/expenses", async (c) => {
  try {
    const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_KEY);
    const { data, error } = await supabase.from("expense_records").select("*").order("created_at", { ascending: false }).limit(100);
    if (error)
      throw error;
    return c.json({ data }, { status: 200 });
  } catch (error) {
...
app.post("/api/expenses", async (c) => {
  try {
    const body = await c.req.json();
    const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_KEY);
    const { data, error } = await supabase.from("expense_records").insert([body]).select();
    if (error)
      throw error;
    return c.json({ data: data?.[0] }, { status: 201 });
  } catch (error) {
...
app.get("/api/dashboard/kpi", async (c) => {
  try {
    ...
    const { count: bookingCount } = await supabase.from("massage_bookings").select("id", { count: "exact", head: true }).gte("booking_date", startOfWeek.toISOString());
    const { data: expenseData } = await supabase.from("expense_records").select("amount").gte("created_at", startOfWeek.toISOString());
...
// .wrangler/tmp/bundle-ArHKcU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ArHKcU/middleware-loader.entry.ts
